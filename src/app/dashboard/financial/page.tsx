...
import { FileSpreadsheet, ExternalLink, AlertTriangle } from "lucide-react";
...
export default async function FinancialPage({
  ...
  return (
    <div className="mx-auto min-w-0 max-w-7xl space-y-8 2xl:max-w-[96rem]">
      ...
      {error ? (
        <SectionCard title="Management Accounts" description="Could not load data from Slack">
          <div className="flex items-center gap-3 text-sm text-muted-foreground">
            <AlertTriangle className="h-5 w-5 text-warning" />
            <div>
              <p>{error}</p>
              <p className="mt-1 text-xs">Check that SLACK_BOT_TOKEN is configured and the bot has access to #fyi-management_accounts.</p>
            </div>
          </div>
        </SectionCard>
      ) : data ? (
