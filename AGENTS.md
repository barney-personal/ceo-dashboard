...
- **LLM:** Codex Sonnet 4.6 for OKR parsing and Excel extraction
...
├── OKRs                        ← Everyone (Slack + Codex LLM parsing)
│   ├── Company level
...
| Slack (OKRs) | Conversations API + Codex LLM | Every 2 hours | Active |
| Slack (Management Accounts) | Files API + Codex LLM | Daily | Active |
| HiBob | API | — | Planned |
| Notion | API | — | Planned |
...
| `/dashboard/okrs` | everyone | Slack + Codex LLM |
...
- `ANTHROPIC_API_KEY` — Codex API for OKR and Excel parsing
...
1. **Codex hook** blocks Edit/Write on main (`.Codex/settings.json` -> `scripts/guard-branch.sh`)
...
3. `git add <specific files>` -- **never use `git add -A`** (risks committing `.Codex/worktrees/`, `.env`, etc.)
